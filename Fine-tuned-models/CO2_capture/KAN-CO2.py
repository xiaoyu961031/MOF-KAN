...
class KAN_Attention(models.Model):
...
kan_attention = KAN_Attention(input_dim)
kan_attention.build(input_shape=(None, None, input_dim))
kan_attention.compile(optimizer=Adam(learning_rate=0.001), loss='mse')
kan_attention.summary()

early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, min_lr=0.001)

history = kan_attention.fit(X_train, y_train, epochs=100, batch_size=32, validation_split=0.2, callbacks=[early_stopping, reduce_lr])

y_test_predict = kan_attention.predict(X_test).flatten()
