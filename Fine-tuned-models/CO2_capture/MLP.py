...
param_grids = {
    'MLP Regressor': {'hidden_layer_sizes': [(512,512,512)], 'activation': ['relu', 'tanh'], 'alpha': [0.0001, 0.001]}
}
...
for name, model in learners.items():
    ...
    rmse = mean_squared_error(y_test_scaled, y_test_pred_scaled, squared=False)
    ...
    test_csv_filename = f'{model_name_sanitized}_test_predictions_.csv'
