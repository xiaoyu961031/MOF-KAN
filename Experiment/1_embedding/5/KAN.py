...
class KAN_GRU(models.Model):
    def __init__(self, input_dim, gru_units):
        super(KAN_GRU, self).__init__()
        self.gru = layers.GRU(gru_units, input_shape=(None, input_dim), kernel_regularizer=regularizers.l2(0.00000001))
...
kan_gru = KAN_GRU(input_dim, gru_units)
kan_gru.build(input_shape=(None, None, input_dim))
kan_gru.compile(optimizer='adam', loss='mse')
kan_gru.summary()

early_stopping = tf.keras.callbacks.EarlyStopping(monitor='val_loss', patience=10, restore_best_weights=True)
reduce_lr = tf.keras.callbacks.ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=5, min_lr=0.001)

history = kan_gru.fit(X_train, y_train, epochs=100, batch_size=32, validation_split=0.2, callbacks=[early_stopping, reduce_lr])

y_test_predict = kan_gru.predict(X_test).flatten()
